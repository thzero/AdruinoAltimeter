...
class sensorsBase {
  ...
  protected:
    virtual void integrateImu(sensorValuesStruct* data, unsigned long current, unsigned long delta);
    virtual void integrateVelocity(sensorValuesStruct* data, unsigned long current, unsigned long delta);
    virtual sensorsSetupResults setupI(sensorsSetupResults results, uint8_t calibrationBarometerId, uint8_t calibrationBarometerStatusId, uint8_t calibrationIMUId, uint8_t calibrationIMUStatusId);

    sensorBarometer* _atmosphereSensor;
    sensorGPS* _gps;
    sensorIMU* _imuSensor;
    bool _initialized;
    float _verticalTolerance = 5.0;
};
